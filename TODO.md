...
appearing all-at-once when the scan finishes. Mirrors the live
progress bars in ZAP's "Active Scan Progress" dialog.
...
3. Internally, run_scan takes an `Option<Arc<RwLock<LiveState>>>` and
   ...
   8.5 passive, 8.7 retire.js, 12 fuzz, completion). About 5-7
...
engineering + docs work; out of scope for v0.8.0 because the
dedicated `cyproxy` tool already covers this surface.
...
OR: drop `cyweb proxy` entirely and route operators to the existing
`cyproxy` tool. Decision deferred to whoever picks this up.

## Live HUD overlay / browser plugin

**What it is:** ZAP's Heads-Up Display — overlay on the operator's
browser as they manually browse the target, showing findings/alerts
in real time.

**Why deferred:** Requires a browser extension or content-script
injection through the proxy. Niche; most operators use the GUI tab
flow.

## Deeper authentication context manager

**What it is:** ZAP-style session re-authentication when a session
expires mid-scan. Right now `--login-user`/`--login-pass` does
form login once at the start; if the session times out during a
2-hour scan, subsequent requests are unauthenticated.
...
## Forced-Browse / dictionary attack

**What it is:** Wordlist-driven path discovery (admin panels,
backup files, common config paths). Cyweb's `--mutate` mode covers
some of this; ZAP's add-on uses larger wordlists (millions of
paths) and a richer rule set.
...
**What it is:** ZAP has Low / Medium / High / Insane strength per
active scan rule. Currently every cyweb rule runs at "Medium"
intensity (one payload per injection point). Insane mode would
fan out 10x more payloads.
...
**What it is:** ZAP lets you toggle individual active-scan rules
on/off. Right now `--tuning` is category-level (passive, fuzz,
retirejs, etc.) — finer-grained per-rule selection would help when
a customer can't run certain rule classes (e.g. avoid SQLi probes
on a production DB).
